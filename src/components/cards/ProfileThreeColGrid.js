...
const CardContent = styled.div`
  ...
  .position {
    ${tw`uppercase font-bold tracking-widest text-xs text-yellow-900`}
  }
  .name {
...
const CardLinks = styled.div`
  ${tw`mt-6 flex`}
  .link {
    ${tw`mr-8 last:mr-0 text-gray-400 hocus:text-red-500 transition duration-300`}
    .icon {
  ...
  return (
    <Container>
      <ContentWithPaddingXl>
        ...
        <Cards>
          {cards.map((card) => (
            <Card key={`${card.name}-${card.position}`}>
              ...
              <CardContent>
                ...
                <CardLinks>
                  {card.links.map((link) => (
                    <a
                      key={`${card.name}-${card.position}-${link.url}`}
                      className="link"
                      href={link.url}
                    >
